...
# To use RTC : lecture : datetime = rtc.read_datetime(), ecriture : rtc.write_datetime(dt_write)
rtc = DS1302(clk_pin=13, data_pin=19, ce_pin=26)
...
MAC_addresses = {
    ...
    'b8:27:eb:1f:ce:1b': 'tom'  # developper 2
}
            ...
            if status == MIFAREReader.MI_OK:
                ...
                if FoolUIDs:
                    try:
                        ...
                        print(
                            "Can't fool UID because of unknown MAC or unregistered fake Card")
...
# Interface that present options to user. options_list array contains 3 characters strings in the following order : yes, no, back
def YesNoBackScreen(line_1, led_state, options_list, defaultOption):
...
# Full Screen AutoScroll, has nothing to see with AutoScroll function below
def FSAutoScroll(String, line):
    j = 0
...
# Multi Line Autoscroll : holds autoscroll during a certain amount of time. Can't be interrupted while autoscrolling, but only between two autoscroll steps
def holdScreen(Strings, ledState, sec, ForbidInterruption):
    mylcd.lcd_clear()
        ...
        for i in range(1, 3):
            ...
            while ForbidInterruption:
                mylcd.lcd_display_string(
                    CenterOnScreen(Strings[i-1][j:j+16], 16), i)
...
# Let user navigate in a level of menu tree
...
# Same as function above, but detached from menu tree
...
# Recursive function to navigate inside main menu
def Menu(firstParentIdentifier, defaultIdentifier):
    global ForceLevel
    global menuTree
    functions_needing_menu_update = [
        "show_attendance_controls", "control_delete"]
    while True:
        print("NAVIGATION INSIDE ", firstParentIdentifier)
        SelectionIdentifier = NavigateLevel(
            firstParentIdentifier, defaultIdentifier)
        ...
        # terminal level, we run requested action. Must implement data handling
        if(SelectionData[1] == ""):
            if "control_" in SelectionIdentifier:
                functionToCall = "control_" + \
                    SelectionIdentifier.split("_", 2)[2]
                argToPass = SelectionIdentifier.split("_", 2)[1]
                globals()[functionToCall](argToPass)
            else:
                # run function whose name corresponds to SelectionIdentifier
                globals()[SelectionIdentifier]()
            SelectionParent = ""
...
def rtc_set_time(dt_to_write):  # Set Real Time Clock module time
    try:
        ...
        if -2 < (dt_to_write - dt_read).total_seconds() < +2:
            print("datetime successfully update on RTC module : ",
                  dt_to_write.strftime('%Y-%m-%dT%H:%M:%SZ'))
            return True
        else:
...
def dismount_USB(mountpoint):  # Allows user to safely remove USB key from Raspberry
    ...
    buzz.warn()
    chx = YesNoBackScreen("EJECTER CLE USB "+mountpoint.split("/")
                          [-1]+" ?", 'y', ['OUI', 'NON'], 'NON')
    if(chx == 'OUI'):
...
# Interface that allows user to set a digit value with a certain pattern
def setDigit(line_1, line_2_pattern, digit_position, default, limit):
    ...
    while not encoder.isClicked():
        ...
        old_count = encoder.getCount()
        while(not encoder.updateCount() and not encoder.isClicked()):
            # allows to blink pattern (perdiod of 2 seconds)
            if(datetime.datetime.now().second % 2 == 0):
                mylcd.lcd_display_string(
                    line_2_pattern[0:digit_position]+str(pos)+" "*(15-digit_position), 2)
            else:
                mylcd.lcd_display_string(
                    line_2_pattern[0:digit_position]+str(pos)+line_2_pattern[digit_position+1:16], 2)
...
# Interface used to navigate set a value with units and subunits (like hour,minutes,seconds). Data must be an array like [[inf,sup,unit],[],...,[]], and default an array like [number, unit]
def setNumberEns(line_1, data, default):
    res = default
    while True:
        inf = 0
        sup = 0
        ...
        res[0] = setNumber(line_1, pattern, positions,
                           [inf, sup], 1, res[0], True)
...
def setNumber(line_1, line_2_pattern, positions, extrema, step, default, ExitIfExtremaReached=False):  # Interface to set a number
    ...
    if default < inf or default > sup:
        ...
        mylcd.lcd_display_string(CenterOnScreen(line_1, 16), 1)
        while not encoder.isClicked():
            pattern = line_2_pattern[0:positions[0]]+str(pos).zfill(
                len(str(extrema[1])))+line_2_pattern[positions[1]+1:16]
...
# Creates a date pattern to display while entering digits
...
# Lists directory content with specific restrictions. Prefer absolute path over relative ones for parentDir
...
def load_students():  # Allows administrator to load the list of students
    list_array = listDirectory(
        '/home/pi/UTBM_TZ20/files/DSI_lists/', False, True, True)
    if len(list_array) != 0:
        list_array = sortDatetimeArray(list_array, FileDatetimeFormat, True)
        # we consider that as an earliest imported file from the same function (thanks to code below), it has the right name and content structures
        dt_current = isDateTime(list_array[0], FileDatetimeFormat)
        if not dt_current:
            print("Error in old DSI_file")
            return
        buzz.warn()
        replace = YesNoBackScreen("ECRASER FICHIER IMPORTE LE "+dt_current.strftime(
            "%d/%m/%Y A %H:%M:%S")+" ?", 'y', ['OUI', 'RET'], 'OUI')
        if replace == 'RET':
            return
    ...
    if len(csv_files) == 0:
        buzz.error()
        holdScreen(["AUCUN FICHIER CSV TROUVE",
                    "SUR LA CLE USB"], 'r', 1, True)
    ...
    print ("CHECKING FILE ", fileToCopy,
           " TO MATCH DSI_file FORMAT : result = ", check_format)
    if not check_format:
        err_filename = err_filename.replace(".csv", ".log")
        buzz.error()
        holdScreen(["ERREUR DE STRUCTURE DU FICHIER", "VOIR " +
                    err_filename.replace(' ', '_')+" SUR CLE USB"], 'r', 1, True)
    else:
        now = rtc_get_time().strftime(FileDatetimeFormat)
        to_delete = listDirectory(
            '/home/pi/UTBM_TZ20/files/DSI_lists', False, True, True)
        if fichier.ImportDSIFile(now):
            for filename in to_delete:
                target = Files()
                target.deleteFile(
                    '/home/pi/UTBM_TZ20/files/DSI_lists/'+filename)
            buzz.success()
            holdScreen(["LISTE ETUDIANTS IMPORTEE",
                        "AVEC SUCCES"], 'g', 1, True)
        else:
            buzz.error()
            holdScreen(["ERREUR LORS DE L'IMPORTATION",
                        "DE LA LISTE DES ETUDIANTS"], 'r', 1, True)
    dismount_USB(USBMountPoint)
...
def set_bip():  # Toggle buzzer
    if buzz.enabled:
        ...
        defaultChoice = "OUI"
    Choice = YesNoBackScreen(
        "BIP SONORE", "m", ['OUI', 'NON', 'RET'], defaultChoice)
    if Choice == 'OUI':
        buzz.setEnable(True)
        led.setColor("g", led)
        buzz.success()
...
def set_datetime():  # Change date and time, either manually or automatially if connected to internet
    choice = YesNoBackScreen("CHOISIR MODE REGLAGE", "m", [
                             "AUT", "MAN", "RET"], "AUT")
    if choice == "AUT":
        isInternet = checkInternet()
        if isInternet:
            holdScreen(["CONNEXION INTERNET OK",
                        "MaJ DATE & HEURE..."], "y", 1, True)
            rtc_set_time(datetime.datetime.now())
            buzz.success()
            holdScreen(
                ["DATE & HEURE", "MISES A JOUR AVEC SUCCES"], "g", 1, True)
        else:
            buzz.error()
            holdScreen(["PAS DE CONNEXION INTERNET",
                        "VEUILLEZ CONNECTER ETHERNET OU METTRE A JOUR MANUELLEMENT"], "r", 1, True)
        ...
        DateDigits = [[6, Digit(DateNumbers[0], 1), 3], [7, Digit(DateNumbers[0], 0), 9], [9, Digit(DateNumbers[1], 1), 1], [10, Digit(DateNumbers[1], 0), 9], [12, Digit(
            DateNumbers[2], 3), 2], [13, Digit(DateNumbers[2], 2), 9], [14, Digit(DateNumbers[2], 1), 9], [15, Digit(DateNumbers[2], 0), 9]]  # field position on screen, value, limit
        newDigits = [[6, -1], [7, -1], [9, -1], [10, -1],
                     [12, -1], [13, -1], [14, -1], [15, -1]]
        for dig in DateDigits:
            pattern = createDatePattern("Date: JJ/MM/AAAA", newDigits)
            newDig = setDigit("MaJ DATE & HEURE", pattern,
                              dig[0], dig[1], dig[2])
            newDigits[DateDigits.index(dig)][1] = newDig
            dig[1] = newDig

        try:
            DateAttempt = datetime.date(Number([DateDigits[4][1], DateDigits[5][1], DateDigits[6][1], DateDigits[7][1]]), Number(
                [DateDigits[2][1], DateDigits[3][1]]), Number([DateDigits[0][1], DateDigits[1][1]]))
        except ValueError:
            setLedState("r")
            buzz.error()
            holdScreen(["ERREUR DATE INCOHERENTE",
                        "VEUILLEZ RECOMMENCER"], "r", 1, True)
            return
        ...
        TimeDigits = [[8, Digit(TimeNumbers[0], 1), 2], [9, Digit(TimeNumbers[0], 0), 9], [11, Digit(TimeNumbers[1], 1), 5], [
            12, Digit(TimeNumbers[1], 0), 9], [14, Digit(TimeNumbers[2], 1), 5], [15, Digit(TimeNumbers[2], 0), 9]]
        ...
        try:
            TimeAttempt = datetime.time(Number([TimeDigits[0][1], TimeDigits[1][1]]), Number(
                [TimeDigits[2][1], TimeDigits[3][1]]), Number([TimeDigits[4][1], TimeDigits[5][1]]))
        except ValueError:
            setLedState("r")
            buzz.error()
            holdScreen(["ERREUR HEURE INCOHERENTE",
                        "VEUILLEZ RECOMMENCER"], "r", 1, True)
            return
...
def scanning(DT=None):  # Launches the scanning procedure
    ...
    if not CheckAdmin():
        ...
        if DT == None:
            now = rtc_get_time()
            UID_list = Files("/home/pi/UTBM_TZ20/files/UID_inputs/" +
                             now.strftime(FileDatetimeFormat)+".csv")
        else:
        ...
        while True:
            ...
            if uid == admin_uid:
                ...
                while(WaitForCard(False, 1) == admin_uid) and count > 0:
                    holdScreen(["MAINTENEZ "+str(count)+" SEC",
                                "POUR QUITTER"], "m", 0.5, True)
                    setLedState("o")
                    time.sleep(0.5)
                    count -= 1
                if count == 0:
                ...
                if not CheckUIDFormat(uid):
                    buzz.error()
                    holdScreen(
                        ["BADGE INCOHERENT", "REESSAYEZ..."], "r", 2, True)
                    ...
                    if UID_list.addStudentToFile(uid, rtc_get_time()):
                        ...
                        buzz.error()
                        holdScreen(
                            [">ERREUR SCAN<", "CARTE DEJA SCANNEE"], "r", 3, True)
                        print ("SCAN N ", nb_scans, " FAILED - UID ",
                               uid, " ALREADY EXISTS IN LIST")
                        nb_rescans += 1
        nb_tot = nb_scans+nb_err+nb_rescans
        buzz.warn()
        result = str(nb_tot)+" SCANS dont "+str(nb_scans)+" SUCCES, " + \
            str(nb_err)+" ERREUR(S) et " + \
            str(nb_rescans)+" TENTATIVE(S) DE RESCAN"
...
def restart():  # Reboot raspberry with confirmation asking
    global IsInterruptionIntentional
    rep = YesNoBackScreen("CONFIRMER REDEMARRAGE ?",
                          "r", ["OUI", "RET"], "RET")
    if(rep == "OUI"):
        buzz.shutDown()
        IsInterruptionIntentional = True
...
def delete_attendance_controls():  # Deletes all attendance controls
    if not CheckAdmin():
        return
    ControlList = listDirectory(
        "/home/pi/UTBM_TZ20/files/UID_inputs", False, True, True)
...
def forbid_wrong_cards():  # Let user set if wrong cards (i.e. cards that are not respecting UTBM format)
    ...
    buzz.warn()
    holdScreen(["CONSEIL : GARDER RESTRICTION ACTIVE",
                "SAUF POUR UTILISATION HORS UTBM"], "y", 1, True)
    choice = YesNoBackScreen("ACCEPTER CARTES UTBM UNIQUEMENT", "y", [
                             'OUI', 'NON', 'RET'], 'OUI')
    if choice == 'OUI':
        acceptUTBMCardsOnly = True
    elif choice == 'NON':
...
def set_interval():  # Let user set the interval of starting time in which two attendance controles are considered to be similar
    ...
    data = [[1, 60, units_on_screen["minutes"]], [
        1, 24, units_on_screen["hours"]], [1, 10, units_on_screen["days"]]]
...
def set_scroll_speed():  # Let user set a scrolling speed for LCD
    global ScrollSpeed
    ScrollSpeed = setNumber("VITESSE DEFIL", "     >   %<     ", [
                            6, 8], [0, 100], 5, ScrollSpeed)
    update_config()
...
def update_system():  # Updates sotfware by performing a git pull
    if not checkInternet():
        buzz.error()
        holdScreen(["PAS DE CONNEXION INTERNET",
                    "VEUILLEZ CONNECTER ETHERNET"], "r", 1, True)
        return
    holdScreen(["RECHERCHE MaJ", "EN COURS..."], 'y', 1, True)
    check_version = subprocess.check_output(
        ['git', 'pull'], cwd="/home/pi/UTBM_TZ20")
    if check_version == 'Already up to date.\n' or check_version == 'D\xc3\xa9j\xc3\xa0 \xc3\xa0 jour.\n':  # English and french returns
        ...
        buzz.warn()
        holdScreen(["REDEMERRAGE OBLIGATOIRE",
                    "POUR APPLIQUER LES MODIFICATIONS"], 'y', 1, True)
...
def get_version():  # Returns the date of last git pull
    date_str = subprocess.check_output(
        ['git', 'log', '-n', '1', '--pretty=format:%ad'], cwd="/home/pi/UTBM_TZ20")
    arr = date_str.split(' ')
    new_str = ' '.join(arr[:-1])
...
def control_extract(DT):  # Performs attendance control extraction
    ...
    try:
        ...
        response = urllib2.urlopen(
            url+ConnectionTestUID).read().decode('utf-8')
    ...
    mode = NavigateInArray(
        "CHOISIR MODE", ["Boitier unique", "Boitiers multiples"], 0, True, 'y')
    ...
    file.compareDsiFilesToFileCreation(
        DSI_absolute, datetime.datetime.strptime(DT, FileDatetimeFormat))
    if not mode:
        dismount_USB(USBMountPoint)
        return
    elif mode == "Boitiers multiples":
        test = Files()
        previous_extraction = test.foundSameEventFile(
            USBMountPoint, datetime.datetime.strptime(DT, FileDatetimeFormat), IntervalMulti)
        if not previous_extraction:
            buzz.error()
            holdScreen(["AUCUNE EXTRACTION VALIDE",
                        "TROUVEE SUR CLE USB"], 'r', 1, True)
            dismount_USB(USBMountPoint)
            return
        buzz.success()
        holdScreen(["EXTRACTION SIMILAIRE",
                    "TROUVEE SUR CLE USB"], 'g', 1, True)
        if YesNoBackScreen("FUSIONNER ?", 'y', ['OUI', 'NON'], "OUI") == "OUI":
            print("Merging files on USB Key")
            holdScreen(["FUSION LISTES", "EN COURS ..."], 'm', 1, True)
            file.addToUSBKEY(USBMountPoint, datetime.datetime.strptime(
                DT, FileDatetimeFormat), IntervalMulti)
            buzz.success()
            holdScreen(
                ["FUSION REUSSIE", "FICHIERS SUR CLE USB"], 'g', 1, True)

    elif mode == "Boitier unique":
        if not file.exportFileToUSB(USBMountPoint, DSI_absolute):
            buzz.error()
            holdScreen(
                ["ERREUR EXPORTATION", "FICHIERS CONTROLE"], 'r', 1, True)
            dismount_USB(USBMountPoint)
            return
        buzz.success()
        holdScreen(
            ["EXPORTATION REUSSIE", "FICHIERS SUR CLE USB"], 'g', 1, True)
    dismount_USB(USBMountPoint)
...
# Shows results of attendance control on LCD. Available only if extraction was performed before
def control_results(datetime):
    Report = Files(
        '/home/pi/UTBM_TZ20/files/Final_extractions/'+datetime+'/report.csv')
    if not Report.exist():
        buzz.error()
        holdScreen(["EXTRACTION MANQUANTE",
                    "FAIRE EXTRACTION PUIS RECOMMENCER"], 'r', 1, True)
        return
    ...
    prefixs = ["Nb Scans", "Nb Attendus",
               "Nb Abs (%)", "Nb Pres (%)", "Nb Faux-Pres (%)", "Duree", "Erreurs API"]
    ret = []
    for res in Results:
...
def control_delete(datetime):  # Deletes targetted attendance control
    ...
    to_delete = ["API_outputs/"+datetime+".csv",
                 "Final_extractions/"+datetime, "UID_inputs/"+datetime+".csv"]
    nb_deletions = 0
    for file in to_delete:
        ...
        if ToDel.deleteFile(file):
            nb_deletions += 1
    print ("DELETED ", nb_deletions,
           " files and folders (only parents are counted)")
    buzz.success()
...
def create_menu():  # Initializes main menu tree
    menu = treelib.Tree()
    # [led_color (o for off), menu_title]
    menu.create_node(identifier='main', data=["", "MENU PRINCIPAL", ""])
    menu.root = 'main'
    menu.create_node(identifier='param', parent='main', data=[
                     "Parametrage", "MENU PARAMETRAGE", "w"])
    menu.create_node(identifier='scanning', parent='main',
                     data=["Lancer scans", "", "g"])
    menu.create_node(identifier='show_attendance_controls', parent='main', data=[
                     "Voir controles", "LISTE CONTROLES", "w"])
    menu.create_node(identifier='delete_attendance_controls', parent='main', data=[
                     "Supprimer tous les controles", "", "r"])
    menu.create_node(identifier='restart', parent='main',
                     data=["Redemarrer", "", "r"])
    menu.create_node(identifier='shutdown', parent='main',
                     data=["Eteindre", "", "r"])
    menu.create_node(identifier='load_students', parent='param',
                     data=["Charger liste etudiants", "", "c"])
    menu.create_node(identifier='set_datetime', parent='param',
                     data=["Date & heure", "", "c"])
    menu.create_node(identifier='set_bip', parent='param',
                     data=["Bip sonore", "", "c"])
    menu.create_node(identifier='set_scroll_speed', parent='param', data=[
                     "Vitesse defilement ecran", "", "c"])
    menu.create_node(identifier='forbid_wrong_cards', parent='param', data=[
                     "Interdire fausses cartes", "", "c"])
    menu.create_node(identifier='set_interval', parent='param',
                     data=["Intervalle mode multiple", "", "c"])
    menu.create_node(identifier='show_MAC', parent='param',
                     data=["Voir adresse MAC Ethernet", "", "c"])
    menu.create_node(identifier='update_system', parent='param',
                     data=["Mise a jour logicielle", "", "g"])
    return menu
...
def sortDatetimeArray(array, format, desc):  # Sort an array of datetime strings
    ...
    for dtstr in array:
        if not isDateTime(dtstr.replace(".csv", ''), format):
            print ("Error with unformatted filename ",
                   dtstr, " in folder files/UID_inputs/")
            return False
        newDTArray.append(datetime.datetime.strptime(
            dtstr.replace(".csv", ''), format))
...
def update_menu():  # Creates as many nodes and subnodes as there are files found in ../files/UID_inputs
    ...
    UID_inputs = listDirectory(
        "/home/pi/UTBM_TZ20/files/UID_inputs/", False, True, True)
    print (len(UID_inputs), " files have been found in UID_inputs : ", UID_inputs)
    UID_inputs = sortDatetimeArray(
        UID_inputs, FileDatetimeFormat, True)  # Descending sort
    for control in UID_inputs:
        ...
        if(dt):
            ...
            try:
                menuTree.create_node(identifier=ctrl_ident, parent='show_attendance_controls', data=[
                                     dtOnScreen, "MENU CONTROLES", 'c'])
            except treelib.exceptions.DuplicatedNodeIdError:
                print ("Node with identifier ", ctrl_ident,
                       " already exists, so can't be created")
            ...
            try:
                menuTree.create_node(identifier=newIdent, parent=ctrl_ident, data=[
                                     "Extraire fichiers", "", 'g'])
            except treelib.exceptions.DuplicatedNodeIdError:
                print ("Node with identifier ", newIdent,
                       " already exists, so can't be created")
...
# associates SIGTERM signal with its handler
...
def check_config():  # Checks validity of mainconfig.ini
    ...
    for expectedOption in ['acceptUTBMCardsOnly', 'buzzEnabled', 'ScrollSpeed', 'IntervalMulti']:
        if not parser.has_option('settings', expectedOption):
            print ("Option ", expectedOption,
                   " was not found in settings section of config file")
            return False
    return True
...
def read_config():  # Parse and read mainconfig.ini
    ...
    if not check_config():
        ...
        return
    acceptUTBMCardsOnly = StrToBool(
        parser.get('settings', 'acceptUTBMCardsOnly'))
